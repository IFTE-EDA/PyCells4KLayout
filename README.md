...
PyCell Enhancements by He Zeng.

Usage Instructions: Place the Python and tech files in the local KLayout directory. Then, load the .lyp file in KLayout.
...
### Added Classes and the Number of Methods Added
...
This is because the variable `Layer.tech` is automatically set to `None` after the first run, preventing further retrieval of information related to `Layer`. If this happens, KLayout must be restarted.
