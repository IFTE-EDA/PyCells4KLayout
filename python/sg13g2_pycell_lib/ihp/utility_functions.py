...
#importlib.reload(cni.dlo)
from cni.dlo.numeric import Numeric
from cni.dlo.location import *
from cni.dlo.orientation import *

nil = 0

tech         = cni.dlo.dlo.Tech.get('SG13_dev')
print(tech)
techparams   = tech.getTechParams()


DEFAULT_EPSILON = 1e-9
SG13_TECHNOLOGY = "SG13G2"
SG13_EPSILON = techparams.get('epsilon1', DEFAULT_EPSILON) # for rounding purposes  #if 'techparams' in globals() and 'epsilon1' in techparams else DEFAULT_EPSILON
...
def fix(value):
    if isinstance(value, float):
        ...
        raise TypeError("fix function supports only int and float types")  
...
#***********************************************************************************************************************
def hiGetAttention():
    print('The set value is out of the normal range')
    print('\a')
...
def CbCapCalc(calc, c, l, w, cell) :
    ...
    global techparams #在您的代码 CbCapCalc 中，global techparams 意味着您打算在这个函数内部使用或修改在函数外部定义的全局变量 techparams。如果 techparams 已在函数外部定义（比如在程序的最上方或在其他地方），这行代码会使得 CbCapCalc 函数能够访问和修改这个全局变量

    caspec = Numeric(techparams[cell+'_caspec'])*1e-12 # specific cap per sq. [um] (float)   1.5m stand for specific area capacitance fF/um square
    cpspec = Numeric(techparams[cell+'_cpspec'])*1e-6  # specific cap. per [um] perimeter (float)  40p stand for specific capacitance MIM perimeter, af/um
