...
#
# Note:
#  He Zeng  Noted an issue with the original code: groupId = ihpCopyFig(groupId, self, Box(2.34, 0), 'R0'). The code has been modified to ihpCopyFig(groupId, Point(2.34, 0), 'R0').
#  HE zeng   Although the groupId is now set by calling ihpCopyFig(groupId, Point(2.34, 0), 'R0'), the layout still presents issues.
########################################################################
    ...
    def setupParams(self, params):
     ...
     def genLayout(self):
        ...
        if Nx > 1 :
            ...
            for cnt in range(Nx) :
                # The code below has been modified, but the resulting layout still has issues
