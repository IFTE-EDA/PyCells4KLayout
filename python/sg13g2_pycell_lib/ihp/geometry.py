...
#   Author        Modification
# ------------  -------------------------------------------------
# He.Zeng      Introduced a function named regionToGroup  used in dbLayerXOr,dbLayerOr,dbLayerXorList : convert pya.Region object to Grouping object
# He.Zeng      Consequently, dbLayerXOr, dbLayerXorList, dbLayerOr, dbLayerAnd, dbLayerAndList, dbLayerNot, dbLayerNotList methods have also been modified to integrate the new 'regionToGroup'  functionality effectively.
#
########################################################################
...
from cni.dlo.path import *

import importlib
importlib.invalidate_caches()

import cni.dlo.text
importlib.reload(cni.dlo.text)
from cni.dlo.text import *

importlib.invalidate_caches()
import cni.dlo.path 
importlib.reload( cni.dlo.path )
from cni.dlo.path  import *
...
from math import *
from cni.dlo.dlo import *
from cni.dlo.dlogen import *

importlib.invalidate_caches()
import cni.dlo.dlogen
importlib.reload(cni.dlo.dlogen)
from cni.dlo.dlogen import *
...
def dbLayerOr(layerId, id1, id2 = None):
    ...
    orRegion = id1.fgOr(id2, layerId) 

    orGroup = regionToGroup(layerId, orRegion)  
    return orGroup
...
def dbLayerAnd(layerId, id1, id2):
    ...
    andRegion = id1.fgAnd(id2, layerId)
    andGroup = regionToGroup(layerId, andRegion)
    return  andGroup
...
def dbLayerAndList(layerId, shapes):
    ...
    idAnd = shapes[0]
    for id in shapes[1:] :
        andRegion = fgAnd(idAnd, id, layerId)
        idAnd = regionToGroup(layerId,andRegion)
...
def dbLayerXor(layerId, id1, id2):
    ...
    xorRegion = id1.fgXor(id2, layerId)
    xorGroup = regionToGroup(layerId, xorRegion)
    return  xorGroup
    
    
def regionToGroup(layerId, region):
    if not isinstance(region, pya.Region):
        raise ValueError("Expected argument is a pya.Region object")
    
    group = Grouping()
    region.merge()
    for item in region.each():
        #print("ITEM",item, type(item),item.is_box(),item.bbox())
        if item.is_box():
            group.add(Rect(layerId, Box(item.bbox().to_dtype(0.001))))

        elif item.holes()==0:
            pointList = []
            for point in item.each_point_hull():
                dpoint = point.to_dtype(0.001)
                pointList.append(Point(dpoint.x, dpoint.y))
            group.add(Polygon(layerId, pointList)) 
        elif item.holes()>0:
            #item1 =item.resolved_holes()
            item.resolve_holes()
            pointList = []
            for point in item.each_point_hull():
                dpoint = point.to_dtype(0.001)
...
def dbLayerXorList(layerId, shapes):
    ...
    xorId = shapes[0]
    for id in shapes[1:] :
        xorRegion = fgXor(xorId, id, layerId)
        
        xorId = regionToGroup(layerId,xorRegion)
...
def dbLayerNot(layerId, id1, id2):
    ...
    notRegion = id1.fgNot(id2, layerId)
    notGroup = regionToGroup(layerId, notRegion)
    return  notGroup
...
def dbLayerNotList(layerId, shapes):
    ...
    notId = shapes[0]
    for id in shapes[1:] :
        notRegion = fgNot(notId, id, layerId)
        
        notId = regionToGroup(layerId, notRegion)
    return notId
...
# dbCreateLabel
#***********************************************************************************************************************    
def dbCreateLabel(self,layer, point, text_content, align, rotation, font, size):
    if not ((isinstance(rotation, str) and rotation in {"R0", "R90", "R180", "R270","MX", "MY", "MXR90", "MYR90"}) or isinstance(rotation, Orientation)):
        raise ValueError("Invalid argument roataion for dbCreateLabel.Supported value are one of 'R0', 'R90', 'R180', 'R270','MX', 'MY', 'MXR90', 'MYR90', or an Orientation instance.")
    if isinstance(rotation, str):
        rotation =strToOrient(rotation)
     
    if isinstance(align, Location):
        nrlocation = align
    elif isinstance(align, str):
         nrlocation = strToAlignt(align)       
    text = Text(layer=layer, text=text_content, origin=point, height=size, location=nrlocation, orient=rotation, font=font)
    return text
...
def dbCreatePin(self, name, shape):
    pin = self.addPin(name, name, shape.bbox, shape.layer)
...
def MkPin(self, termName, termIndex, bBox, layerId, label=False, labelHight = -1):
    ...
    self.addPin(termName, termName, bBox, layerId)  #In addition, this method will also create thegeometry for this pin on the layer (or layers) specified by the layer parameter, using the rectangle specified by the box Box parameter
